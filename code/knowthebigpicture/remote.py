...
def remote_root_from_env():
    return os.environ.get("KBP_REMOTE_ROOT")


def rclone_bin_from_env():
    return os.environ.get("KBP_RCLONE_BIN", "rclone")
...
def configure_rclone_from_secret():
    ...
    config_path = BASE_DIR / ".kbp_runtime" / "rclone.conf"
...
def run_rclone_copy(rclone_bin, source, destination, label, exclude=None):
    ...
    try:
        ...
        raise KtwError(f"rclone failed while {label.lower()}") from e
...
def rclone_copy_file(rclone_bin, local_file, remote_path):
    ...
    try:
        ...
        raise KtwError(f"rclone failed to copy {local_file} to {remote_path}") from e
...
def rclone_public_link(rclone_bin, remote_path):
    ...
    if not link:
        raise KtwError(f"rclone link returned no URL for {remote_path}")
    return link
...
def push_remote_job(job_id, remote_root, jobs_root, rclone_bin):
    ...
    if not local_job.exists():
        raise KtwError(f"Local job folder does not exist, cannot push: {local_job}")
