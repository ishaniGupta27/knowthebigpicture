...
def parse_args(argv):
    parser = argparse.ArgumentParser(
        description="Know the Big Picture: build a question-led educational Short."
    )
    ...
    parser.add_argument(
        ...
        "Optionally pass a remote root (e.g. gdrive:knowthebigpicture).",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Ignore cached explainer.json and regenerate the explanation.",
    )
...
def resolve_remote_root(value):
    ...
    if value == "__env__":
        ...
        if not root:
            raise KtwError(
                "--remote given without a root and KBP_REMOTE_ROOT is not set."
            )
        return root
    return value
