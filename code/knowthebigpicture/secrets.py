...
DEFAULT_SECRETS_PATH = BASE_DIR / "secrets" / "knowthebigpicture.secrets.json"
...
def load_secret_file(path=None):
    ...
    try:
        ...
        raise KtwError(f"Invalid secrets JSON: {secret_path}: {e}") from e

    if not isinstance(data, dict):
        raise KtwError(f"Secrets file must contain a JSON object: {secret_path}")
