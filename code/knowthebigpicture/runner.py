...
def run_job(
    ...
    log_section("KNOW THE BIG PICTURE JOB")
    ...
    try:
        ...
        log_section("STAGE 1  EXPLAIN")
        explainer = mock_parse(job, force=force) if dry_run else run_parse(job, force=force)

        log_section("STAGE 2  VERIFY")
        explainer = run_verify(job, explainer)

        log_section("STAGE 2b  CONTENT")
        content_path = write_content(job, explainer)
        ...
        if not force and job.video_path.is_file():
            ...
            image_results = (
                ...
                if dry_run
                else run_images(job, explainer, force=force)
            )
