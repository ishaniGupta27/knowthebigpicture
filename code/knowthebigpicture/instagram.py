...
def validate_instagram_config(job):
    ...
    if not instagram.get("enabled", False):
        raise KtwError("instagram.enabled must be true to publish")
...
def resolve_video_url(job, remote_root=None, rclone_bin=None):
    ...
    if not remote_root:
        raise KtwError(
            ...
            "in job.json, or run with --remote (KBP_REMOTE_ROOT) so the video is "
            "on Drive and a public link can be generated."
        )

    rclone_bin = rclone_bin or rclone_bin_from_env()
    remote_path = remote_join(remote_root, "jobs", job.job_id, "outputs", "explainer.mp4")
...
def create_reel_container(base, ig_user_id, video_url, caption, token):
    ...
    if not creation_id:
        raise KtwError("Instagram media container response did not include an id")
    return creation_id
...
def publish_container(base, ig_user_id, creation_id, token):
    ...
    if not media_id:
        raise KtwError("Instagram media_publish response did not include an id")
    return media_id
