...
def fit_heading_font(draw, text, max_width, max_lines=4):
    """Largest heading font that fits the educational slide lockup."""
...
def build_lockup(node, cfg, draw):
    ...
    heading_font = fit_heading_font(draw, heading, max_width)
    kicker_font = load_font(int(W * 0.036))
    detail_font = load_font(int(W * 0.042))

    lines = []

    if cfg["show_role_kicker"] and node.get("role") != settings.ROLE_QUESTION:
        lines.append(
            (
                node["role"].replace("_", " ").upper(),
                kicker_font,
                (214, 214, 214, 255),
                18,
            )
    ...
    for line in wrap_text(draw, heading, heading_font, max_width):
        lines.append((line, heading_font, (255, 255, 255, 255), 8))
    if lines:
        text, font, color, _ = lines[-1]
        lines[-1] = (text, font, color, 0)

    if cfg.get("show_explanation") and node.get("explanation"):
        text, font, color, _ = lines[-1]
        lines[-1] = (text, font, color, 26)
        for line in wrap_text(draw, node["explanation"], detail_font, max_width):
            lines.append((line, detail_font, (255, 255, 255, 255), 8))
        text, font, color, _ = lines[-1]
        lines[-1] = (text, font, color, 0)
...
def compose_node(node, background_path, out_path, cfg):
    if not background_path.is_file():
        raise KtwError(f"Background missing for node {node['id']}: {background_path}")
...
def run_compose(job, explainer):
    ...
    frames = []
    for node in explainer.get("slides", []):
        node_id = node["id"]
