...
def validate_youtube_config(job):
    ...
    if not youtube.get("enabled", False):
        raise KttError("youtube.enabled must be true to publish")

    if youtube.get("upload_type", "short") != "short":
        raise KttError("youtube.upload_type must be 'short'")

    if youtube.get("privacy_status", "private") != "private":
        raise KttError("youtube.privacy_status must be 'private'")
...
def validate_short_video(video_path):
    ...
    if not path.is_file():
        raise KttError(f"Video file not found: {path}")
    if path.suffix.lower() != ".mp4":
        raise KttError(f"YouTube Shorts upload requires an mp4 file: {path}")
    ...
    if duration <= 0:
        raise KttError("YouTube Shorts upload requires a valid video duration")
...
def access_token():
    ...
    if not token:
        raise KttError("YouTube OAuth response did not include access_token")
    return token
...
def initiate_upload(video_path, metadata, youtube, token):
    ...
    if not upload_url:
        raise KttError("YouTube upload session response did not include Location")
...
def publish_short(job, force=False):
    ...
    if not video_id:
        raise KttError("YouTube upload response did not include video id")
